...
# Define all available sessions with their data files
SESSIONS = {
    "Cybersecurity": {
    ...
    #     "data_file": "sls_kpi_data_marketing.json",
    #     "color": "#3498db",
    #     "icon": "📢"
    # }
}
...
# If no session selected, show selection screen
if st.session_state.selected_session is None:
    ...
    st.markdown("### Select a Session to View Analytics")
    st.markdown("---")
    ...
    for idx, (session_key, session_info) in enumerate(SESSIONS.items()):
        with cols[idx]:
            ...
            st.markdown(f"""
            <div style='
                border: 2px solid {session_info['color']};
                border-radius: 10px;
                padding: 1rem;
                margin-top: 0.5rem;
                text-align: center;
            '>
                <h4 style='color: {session_info['color']};'>{session_info['icon']}</h4>
                <p style='font-size: 0.9rem;'>Click to view analytics</p>
            </div>
            """, unsafe_allow_html=True)
...
# Add a "Back to Sessions" button at the top
...
with col1:
    if st.button("← Back to Sessions", type="secondary"):
        ...
        st.rerun()
with col2:
    st.markdown(f"<h2 style='text-align: center; color: {session_info['color']};'>{session_info['name']}</h2>", unsafe_allow_html=True)
...
if data is None:
    st.error(f"Could not load data for {selected_session} session. Please check if the data file exists.")
...
# Event attendance constants - now loaded from session data
...
with tab2:
    ...
    funnel_stages = ['Knowledge Improved', 'Planning Action']
    funnel_values = [
