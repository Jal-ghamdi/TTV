...
import numpy as np # ============================================================================ # PAGE CONFIGURATION # ============================================================================ 
st.set_page_config(
...
INITIATIVES = {
    "Towards the Vision": {
        ...
        "icon": "🎯",
        "description": (
            "Educational initiative empowering participants with knowledge "
            "and skills aligned with Vision 2030 goals"
        ),
        "color": "#006341",
        "sessions": {
            "Cybersecurity": {
    ...
    "Misk Tracks": {
        ...
        "icon": "🚀",
        "description": (
            "An initiative highlighting Misk Foundation's four tracks — "
            "Leadership, Entrepreneurship, Skills, and Community — to help "
            "Saudi students in Australia explore programs aligned with their "
    ...
    "Leaders Network Circles": {
        ...
        "icon": "🔗",
        "description": (
            "Professional networking circles connecting Saudi students and "
            "professionals in Australia — Growth, Connection, and Impact "
            "circles — aligned with Vision 2030."
        ),
        "color": "#0d3b6e",
        "sessions": {
            ...
            "Technology Sector Session 1": {
                ...
                "data_file": None,
                "icon": "💻",
                "vision_theme": "Tech Networking & Digital Innovation",
                "color": "#6366f1",
                "topic_labels": [],
                "type": "lnc",
            },
...
st.markdown(
    ...
    <style>
        @import url(
            'https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@300;400;500;600;700&family=Epilogue:wght@300;400;500;600;700;800;900&display=swap'
        );
        ...
        * {
        ...
        .main {
            background:
                linear-gradient(
                    135deg,
                    rgba(0, 132, 61, 0.03) 0%,
                    rgba(147, 193, 63, 0.03) 100%
                ),
                repeating-linear-gradient(
                    45deg,
                    transparent,
                    transparent 10px,
                    rgba(0, 99, 65, 0.01) 10px,
                    rgba(0, 99, 65, 0.01) 20px
                );
...
def lnc_bar(
    ...
    if v_range:
        layout['yaxis'] = dict(range=v_range)

    if h_range:
        layout['xaxis'] = dict(range=h_range)
...
def compare_band(
    ...
    if isinstance(pre_val, (int, float)) and isinstance(post_val, (int, float)):
        ...
        delta_str = (
            f"+{delta:.1f}{unit}"
            if delta > 0
            else f"{delta:.1f}{unit}"
        )
        delta_class = (
            "delta-pos"
            if delta > 0
            else ("delta-neg" if delta < 0 else "delta-neu")
        )
    else:
    ...
    <div class="compare-band">
        ...
        {
            "<div class='compare-col' style='flex:0.7'><div class='compare-sub' style='font-size:0.8rem;color:#888;padding-top:1rem'>"
            + note +
            "</div></div>"
            if note else ""
        }
...
if st.session_state.selected_initiative is None:
    ...
    <div class="info-box">
        ...
        <p>
            The Saudi Leadership Society Australia Chapter runs multiple initiatives
            to empower Saudi students and professionals with knowledge, skills, and
            connections aligned with <strong>Vision 2030</strong> goals.
        </p>
    </div>
    """, unsafe_allow_html=True)

    st.markdown(
        '<p class="section-title">🌟 Select an Initiative</p>',
        unsafe_allow_html=True
    )
...
if st.session_state.selected_session is None:
    ...
    st.markdown(
        ...
        <div class="sls-header">
            <h1 class="initiative-title">
                {initiative_info['icon']} {initiative_key}
            </h1>
            <p class="initiative-subtitle">
                {initiative_info['description']}
            </p>
        </div>
        """,
        unsafe_allow_html=True
    ...
    st.markdown(
        '<p class="section-title">📚 Select a Session</p>',
        unsafe_allow_html=True
    )
        ...
        with cols[col_idx]:
            ...
            st.markdown(
                ...
                <div class="session-card">
                    ...
                    <p class="session-theme">
                        {session_info['vision_theme']}
                    </p>
                </div>
                """,
                unsafe_allow_html=True
...
# ─── Determine LNC sub-type ──────────────────────────────────────────────────

IS_LNC_TECH = (
    session_info.get('type') == 'lnc'
    and selected_session == "Technology Sector Session 1"
)

IS_LNC_HEALTH = (
    session_info.get('type') == 'lnc'
    and selected_session == "Health Sector Session 1"
)
...
if IS_LNC_TECH:
    ...
    st.markdown(
        f"""
        <div class="lnc-tech-header">
            <h1 class="initiative-title">💻 Leaders Network Circles</h1>
            <p class="initiative-subtitle">
                Saudi Leadership Society — Australia Chapter
            </p>

            <div style="text-align:center; margin-top:1.2rem; position:relative; z-index:1;">
                <span class="circle-badge circle-tech">🌱 Growth</span>
                <span class="circle-badge circle-tech">🤝 Connection</span>
                <span class="circle-badge circle-tech">💥 Impact</span>
            </div>
    ...
    st.markdown(
        ...
        <div class="sls-header">
            <h1 class="initiative-title">
                {session_info["icon"]} {selected_session}
            </h1>
            <p class="initiative-subtitle">
                {initiative_info['name']} • {session_info['vision_theme']}
            </p>
        </div>
        """,
        unsafe_allow_html=True
...
# ============================================================================
# TECHNOLOGY SECTOR LNC DASHBOARD
# ============================================================================

if IS_LNC_TECH:

    N = 22  # total respondents (19 Excellent + 2 Good + 1 Fair = 22 on Q1)
    ...
    satisfied_high = post['satisfaction']['Very satisfied (5)'] + post['satisfaction']['Satisfied (4)']
    satisfied_pct = round(satisfied_high / N * 100)

    discussion_helpful = post['discussion_qs']['Very helpful'] + post['discussion_qs']['Helpful']
    discussion_pct = round(discussion_helpful / N * 100)

    # ── TOP STAT PILLS ────────────────────────────────────────────────────────
    st.markdown(f"""
    <div class="lnc-tech-highlight-box" style="padding:2rem;">
        <div style="display:flex; flex-wrap:wrap; justify-content:center; gap:0.5rem; position:relative; z-index:1;">
            ...
            <div class="stat-pill"><span class="num">{format_excel_pct}%</span><span class="lbl">Excellent rating</span></div>
            <div class="stat-pill"><span class="num">{round(conf_post_high/N*100)}%</span><span class="lbl">Confidence boosted</span></div>
    ...
    with c2:
        st.markdown(
            create_kpi_card(
                "FORMAT",
                "Circle Format — Excellent",
                f"{format_excel_pct}%",
                f"{post['format_rating']['Excellent']} out of {N} rated Excellent",
                "✓ Circle-rotation innovation"
            ),
            unsafe_allow_html=True
        )

    with c3:
        reg_pre_n = sum(pre['registration'].values())  # 24 pre-respondents
        reg_excel_pct = round(pre['registration']['Excellent'] / reg_pre_n * 100)
    ...
    with c1:
        st.markdown(
            create_kpi_card(
                ...
                f"{post['confidence']['Very confident']} very confident + {post['confidence']['Confident']} confident",
                f"✓ Up from {round(conf_pre_high/sum(pre['confidence'].values())*100)}% pre-event"
            ),
            unsafe_allow_html=True
        )
    ...
    with c2:
        linkedin_6plus = (
            post['linkedin_conns']['6–9'] +
            post['linkedin_conns']['10–12'] +
            post['linkedin_conns']['12+']
        )

        st.markdown(
            create_kpi_card(
                "CONNECTIONS",
                "Made 6+ LinkedIn Connections",
                f"{round(linkedin_6plus/N*100)}%",
                f"{linkedin_6plus} out of {N} participants",
    ...
    with c3:
        st.markdown(
            create_kpi_card(
                ...
                f"{relevant_pct}%",
                "Rated connections Very Relevant or Relevant",
    ...
    with c1:
        st.markdown(
            create_kpi_card(
                ...
                "Highly Satisfied",
                f"{satisfied_pct}%",
                f"{satisfied_high} rated 4–5 stars out of {N}",
                "✓ Outstanding quality"
            ),
            unsafe_allow_html=True
        )
    ...
    with c2:
        st.markdown(
            create_kpi_card(
                ...
                "Net Promoter Score",
                str(nps_score),
                f"{post['nps']['Promoters']} promoters · {post['nps']['Passives']} passives · {post['nps']['Detractors']} detractors",
                "✓ Strong advocacy"
            ),
            unsafe_allow_html=True
        )
    ...
    # ── HIGHLIGHT BOX ─────────────────────────────────────────────────────────
    st.markdown(f"""
    <div class="lnc-tech-highlight-box">
        <h3>✨ Technology Sector Session — Highlights</h3>
        <ul>
            <li>{format_excel_pct}% rated the circle-rotation format as <strong>Excellent</strong> — format validated again</li>
            <li>Professional tech network goal over-delivered: 19 aimed → 21 achieved (+10.5%)</li>
            <li>Overwhelming atmosphere dropped to <strong>0%</strong> post-event — circles removed friction</li>
            <li>{round(linkedin_6plus/N*100)}% of participants made <strong>6 or more</strong> LinkedIn connections</li>
            <li>{discussion_pct}% found the per-round discussion questions <strong>helpful or very helpful</strong></li>
        </ul>
    </div>
    """, unsafe_allow_html=True)
    ...
    with tab1:
        ...
        st.markdown(
            compare_band(
                round(pre['atmosphere']['Comfortable\n& friendly'] / sum(pre['atmosphere'].values()) * 100),
                round(post['atmosphere']['Comfortable\n& friendly'] / N * 100),
                f"{pre['atmosphere']['Comfortable & friendly'.replace('& ', '& ')]} people felt comfortable before",
                f"{post['atmosphere']['Comfortable\n& friendly']} people felt comfortable after",
                "#6b7280",
                "#4f46e5",
                "%",
        ...
        pre_atm_n = sum(pre['atmosphere'].values())
        ...
        with c1:
            sat_labels = list(post['satisfaction'].keys())
            sat_values = list(post['satisfaction'].values())
            sat_colors = ['#312e81', '#4f46e5', '#818cf8', '#e74c3c', '#c0392b']
        ...
        with c2:
            ...
            dq_labels = list(post['discussion_qs'].keys())
            dq_values = list(post['discussion_qs'].values())
            dq_colors = ['#312e81', '#4f46e5', '#818cf8', '#e9ecef', '#e74c3c']

            fig = lnc_bar(dq_labels, dq_values, dq_colors, height=320, v_range=[0, 15])

            st.plotly_chart(fig, use_container_width=True)

        st.markdown("### Recommendation to Other Saudi Students")

        rec_labels = list(post['recommendation'].keys())
        rec_values = list(post['recommendation'].values())
        rec_colors = ['#312e81', '#4f46e5', '#818cf8', '#e74c3c', '#c0392b']
    ...
    with tab2:
        ...
        st.markdown(
            compare_band(
                ...
                "#4f46e5",
                "",
                "Tech network goal exceeded"
            ),
            unsafe_allow_html=True
        )

        aim_labs = list(pre['conn_type_aim'].keys())
        gain_labs = list(post['conn_type_gained'].keys())

        fig = lnc_grouped_bar(
            aim_labs,
            [pre['conn_type_aim'][k] for k in aim_labs],
            [post['conn_type_gained'][k] for k in gain_labs],
            post_color='#4f46e5',
            y_max=25
        )

        fig.update_layout(
            title_text="Connection Types: Intended (Pre) vs Achieved (Post)",
        ...
        with c1:
            ...
            fig = lnc_bar(
                list(post['meaningful_conns'].keys()),
                list(post['meaningful_conns'].values()),
                '#312e81',
                height=300,
                v_range=[0, 12]
            )
        ...
        with c2:
            st.markdown("#### Connection Relevance")

            fig = lnc_donut(
                list(post['relevance'].keys()),
                list(post['relevance'].values()),
                ['#312e81', '#4f46e5', '#818cf8', '#e74c3c', '#c0392b'],
    ...
    with tab3:
        ...
        st.markdown(
            compare_band(
                round(conf_pre_high / sum(pre['confidence'].values()) * 100),
                round(conf_post_high / N * 100),
                "were confident or very confident before",
                "are confident or very confident after",
                "#6b7280",
                "#4f46e5",
                "%",
                "Confidence growth"
        ...
        pre_conf_n = sum(pre['confidence'].values())
        ...
        with c2:
            ...
            nps_vals = [post['nps'][k] for k in nps_labs]
            nps_colors = ['#312e81', '#818cf8', '#e74c3c']

            fig = lnc_donut(nps_labs, nps_vals, nps_colors, center_text=f"NPS\n{nps_score}")
        ...
        fig.update_layout(
            title_text="Networking Barriers: Pre-Event vs Overcome (Post-Event)",
            title_font=dict(family='Cormorant Garamond', size=18)
        )
    ...
    with tab4:
        ...
        pre_conf_high_pct = round(conf_pre_high / sum(pre['confidence'].values()) * 100)
        ...
        metrics_compare = [
            ...
            (
                ...
                round(pre['atmosphere']['A bit\noverwhelming'] / sum(pre['atmosphere'].values()) * 100),
                round(
                    post['atmosphere']['A bit\nowerwhelming'] / N * 100
                    if 'A bit\nowerwhelming' in post['atmosphere']
                    else post['atmosphere']['A bit\noverwhelming'] / N * 100
                ),
                "%"
            ),
        ]
    ...
    with tab5:
        ...
        with c2:
            ...
            fig = lnc_bar(
                ...
                ['#312e81', '#4f46e5', '#818cf8', '#f59e0b', '#e9ecef'],
                height=340,
                v_range=[0, 12]
            )
        ...
        st.markdown("#### What Type of Connections Did Participants Want to Build?")

        aim_labs_clean = [k.replace('\n', ' ') for k in pre['conn_type_aim'].keys()]

        fig = lnc_bar(
            aim_labs_clean,
            list(pre['conn_type_aim'].values()),
            ['#312e81', '#4f46e5', '#818cf8', '#a5b4fc', '#c7d2fe'],
            height=300,
            v_range=[0, 23]
        )
        ...
        st.markdown("#### Main Networking Barriers Faced Before the Event")
...
<p>
The <strong>Technology Sector session</strong> is the second session under Leaders Network Circles, the third initiative of the Saudi Leadership Society Australia Chapter. It connects Saudi students and professionals in the technology sector through a structured <strong>circle-rotation format</strong> — short rounds of focused conversations across three circles: <strong>Growth, Connection, and Impact</strong>. Participants build genuine tech-sector relationships, explore cross-sector opportunities (gov-tech, fintech, health-tech), and strengthen the Saudi digital community in Australia — aligned with <strong>Vision 2030's</strong> Digital Transformation goals.
<br><br>
<strong>Eligibility:</strong> Saudi nationals studying or working in technology-related fields in Australia.
</p>
</div>
""", unsafe_allow_html=True)
...
# ── FOOTER ────────────────────────────────────────────────────────────────
st.markdown(f"""
<div class="lnc-tech-footer">
<h2>💻 Leaders Network Circles</h2>
<p class="tagline">Technology Sector — Session 1 · Australia Chapter</p>
...
<div><div style="font-size:2.5rem;font-weight:700">{format_excel_pct}%</div><div style="opacity:0.8">Excellent rating</div></div>
...
<div><div style="font-size:2.5rem;font-weight:700">{relevant_pct}%</div><div style="opacity:0.8">Relevant connections</div></div>
</div>
...
# ============================================================================
# HEALTH SECTOR LNC DASHBOARD (unchanged from original)
# ============================================================================
...
if IS_LNC_HEALTH:
    ...
    pre = dict(
        ...
        confidence = {"Extremely confident": 12, "Somewhat confident": 5, "Neutral": 9, "Somewhat not confident": 1, "Extremely not confident": 0},
        barriers = {"Difficulty finding\nrelevant people": 10, "Not knowing how to\nstart conversations": 8, "Shyness / anxiety": 6, "Other": 3},
        conn_targets = {"1–2": 8, "3–5": 9, "6–9": 5, "10–12": 2, "12+": 3},
        goals = {"Friendships": 20, "Professional\nnetwork": 21, "Mentor": 13, "Research /\nprojects": 10, "Saudi\ncommunity": 14},
        atmosphere = {"Comfortable\n& friendly": 17, "Professional &\nwell-organized": 11, "A bit\noverwhelming": 6, "Hard to\nconnect": 0, "Casual & not\norganized": 1},
    ...
    try:
        ...
        with col_c:
            st.image(
                "d3b23000-f8ed-4a60-a6c9-14ff54a1a604.JPG",
        ...
        try:
            ...
            with col_c:
                st.image(
                    "/mnt/user-data/uploads/d3b23000-f8ed-4a60-a6c9-14ff54a1a604.JPG",
                    caption="Leaders Network Circles — Health Sector, Session 1",
                    use_column_width=True
                )
        except:
    ...
    with c2:
        st.markdown(
            create_kpi_card(
                "FORMAT",
                "Circles Format — Excellent",
                f"{round(post['format_rating']['Excellent']/N*100)}%",
                f"{post['format_rating']['Excellent']} out of {N} rated Excellent",
                "✓ Innovative circle-rotation format"
            ),
            unsafe_allow_html=True
        )
    ...
    with c3:
        ...
        st.markdown(
            create_kpi_card(
                ...
                "Significantly expanded their health-sector network",
                "✓ Vision 2030 aligned"
            ),
            unsafe_allow_html=True
        )
    ...
    with c1:
        ...
        st.markdown(
            create_kpi_card(
                ...
                f"✓ +{exp_vg - pre['experience']['Very Good']} improvement"
            ),
            unsafe_allow_html=True
        )
    ...
    with c2:
        st.markdown(
            create_kpi_card(
                ...
                f"✓ +{nps_post_score - nps_pre_score} point jump"
            ),
            unsafe_allow_html=True
        )
    ...
    with tab1:
        ...
        with c2:
            ...
            fig.add_trace(go.Bar(
                ...
                text=[pre['nps'][k] for k in nps_labs],
                textposition='outside'
            ))
            fig.add_trace(go.Bar(
                name='After',
    ...
    with tab2:
        ...
        st.markdown(compare_band(
            21,
            24,
            "aimed to expand professional network",
            "actually expanded professional network",
            "#6b7280",
            "#0d3b6e",
            "",
        ...
        fig.update_layout(title_text="Goals: Intended (Pre) vs Achieved (Post)", title_font=dict(family='Cormorant Garamond', size=18))
        st.plotly_chart(fig, use_container_width=True)
    ...
    with tab3:
        ...
        fig.update_layout(title_text="Barriers: Pre-Event vs Overcome (Post-Event)", title_font=dict(family='Cormorant Garamond', size=18))
        st.plotly_chart(fig, use_container_width=True)
    ...
    with tab4:
        ...
        metrics_compare = [
            ("Comfortable atmosphere", round(pre['atmosphere']['Comfortable\n& friendly']/N*100), round(post['atmosphere']['Comfortable\n& friendly']/N*100), "%"),
            ("Very Good overall experience", round(pre['experience']['Very Good']/N*100), round(post['experience']['Very Good']/N*100), "%"),
            ("Net Promoter Score", nps_pre_score, nps_post_score, "pts"),
            ("Confidence (highly confident)", round((pre['confidence']['Extremely confident'])/N*100), round(post['confidence_change']['Significantly\nimproved']/N*100), "%"),
            ("Overwhelming atmosphere", round(pre['atmosphere']['A bit\noverwhelming']/N*100), round(post['atmosphere']['A bit\noverwhelming']/N*100), "%"),
        ]
    ...
    <div class="info-box">
        ...
        <p>
            Leaders Network Circles is the <strong>third initiative</strong> of the Saudi Leadership Society Australia Chapter. It brings together Saudi health students and professionals through a structured <strong>circle-rotation format</strong> — six short rounds of small-group conversations across three circles: <strong>Growth, Connection, and Impact</strong>.
            Participants build genuine relationships, explore career pathways, and strengthen the Saudi health community abroad — all aligned with <strong>Vision 2030</strong>'s health sector goals.
            <br><br>
            <strong>Eligibility:</strong> Saudi nationals studying or working in the Australian health sector. Limited to 30 leaders per session.
        </p>
    </div>
    """, unsafe_allow_html=True)
...
# ============================================================================
# HEALTH SESSION DASHBOARD (JSON-driven, type == 'health')
# ============================================================================
if session_info.get('type') == 'health':

    metrics = data['metrics']
    topics = session_info['topic_labels']
    topic_keys = session_info.get(
        'topic_keys',
        ['grow_sector', 'grow_vision2030', 'grow_job_market', 'grow_skills']
    )
    ...
    if improvements[best_idx] > 0:
        achievements.append(
            f"Strongest gain in '{topics[best_idx]}': +{improvements[best_idx]:.2f} pts"
        )

    if positive_topics >= 3:
        achievements.append(
            f"Positive growth recorded in {positive_topics} out of {len(topics)} topic areas"
        )

    if avg_post >= 3.5:
        achievements.append(
            f"Post-session average of {avg_post:.2f}/5 across all health topics"
        )
        ...
        if not location_data and not heard_data and not academic_data:
                ...
                if location_data:
                    fig_loc = go.Figure(
                        data=[go.Pie(
                            ...
                            hole=0.45,
                            marker_colors=[
                                '#006341', '#00843d', '#93c13f', '#b8d96d',
                                '#d4e89e', '#e9f5c9', '#f4fbe8'
                            ],
                            textfont=dict(size=13, family='Epilogue'),
                            textinfo='label+percent'
                        )]
            ...
            with col2:
                ...
                if heard_data:
                    ...
                    fig_heard = go.Figure(data=[go.Bar(
                        ...
                        y=heard_values,
                        marker_color=[
                            '#006341', '#00843d', '#93c13f', '#b8d96d', '#d4e89e'
                        ][:len(heard_labels)],
                        text=heard_values,
                        textposition='outside',
            ...
            st.markdown(
                ...
                <div class="info-box">
                    ...
                    <p>This <strong>{selected_session}</strong> session is part of the <strong>{initiative_info['name']}</strong> initiative, designed to deepen participants' understanding of the {selected_session} sector landscape, its role in <strong>Vision 2030</strong>, and the skills and job market opportunities it presents. Survey data was collected before and after the session to measure knowledge growth across four core topic areas: {', '.join(topics)}.</p>
                </div>
                """,
                unsafe_allow_html=True
            )
...
if session_info.get('type') == 'awareness':
    ...
    with col2:
        ...
        <div class="kpi-card">
            ...
            <div class="kpi-context">Participants aware of Misk Tracks</div>
            <div class="kpi-trend">✓ {metrics['awareness_summary']['awareness_increase_pct_points']:.1f} pp increase</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    <div class="info-box">
        ...
        <p>This baseline awareness assessment was conducted to understand participants' familiarity with <strong>Misk Tracks</strong> before and after the session.</p>
    </div>
    """, unsafe_allow_html=True)
...
if session_info.get('type') == 'comprehensive':
    ...
    if overall_increase >= 80:
        achievements.append(f"Massive {overall_increase:.1f} pp awareness increase (overall)")

    if paired_increase >= 80:
        achievements.append(f"True individual growth: {paired_increase:.1f} pts for matched participants")
...
with col3:
    ...
    st.markdown(create_kpi_card(
        ...
        "✓ Exceeded target" if ACTUAL_ATTENDEES>TARGET else "→ Approaching target"
    ), unsafe_allow_html=True)
...
with col1:
    ...
    st.markdown(create_kpi_card(
        ...
        "✓ Strong growth" if avg_growth>=1.0 else "→ Moderate growth"
    ), unsafe_allow_html=True)
...
with col2:
    ...
    st.markdown(create_kpi_card(
        ...
        "✓ Excellent reach" if improved_pct>=70 else "→ Good reach"
    ), unsafe_allow_html=True)
...
with col3:
    ...
    st.markdown(create_kpi_card(
        ...
        "Gained ≥0.5 points",
        "✓ Deep learning" if significant_pct>=50 else "→ Solid progress"
    ), unsafe_allow_html=True)
...
with col1:
    ...
    st.markdown(create_kpi_card(
        ...
        "✓ Outstanding" if action_pct>=80 else "✓ Strong" if action_pct>=60 else "→ Growing"
    ), unsafe_allow_html=True)
...
with col2:
    ...
    st.markdown(create_kpi_card(
        ...
        "Average participant rating",
        "✓ Excellent" if satisfaction>=4.5 else "✓ Very good" if satisfaction>=4.0 else "→ Good"
    ), unsafe_allow_html=True)
...
with col3:
    ...
    st.markdown(create_kpi_card(
        ...
        f"{satisfied_pct:.0f}%",
        "Rated 4+ stars",
        "✓ Strong approval" if satisfied_pct>=70 else "→ Positive reception"
    ), unsafe_allow_html=True)
...
if achievements:
    st.markdown(f"""
    <div class="highlight-box"><h3>✨ Session Highlights</h3>
    <ul>{''.join([f'<li>{a}</li>' for a in achievements])}</ul></div>
    """, unsafe_allow_html=True)
